...
const DiagonalsParallelogram = ({ customData }) => {
  ...
  const checkDiagonals = () => {
    ...
    // Define all correct diagonal pairs (in both possible directions)
    const correctLines = [
      [correctDiagonals[0], correctDiagonals[1]],
      [correctDiagonals[1], correctDiagonals[0]]
    ];
  
    // Check each drawn line against all correct diagonal pairs
    const matchedLines = lines.map(line => {
      const isMatch = correctLines.some(([correctLine, reversedLine]) => 
        isCloseEnough(line.points, correctLine) || isCloseEnough(line.points, reversedLine)
      );
      return isMatch;
    });
  
    // If all lines are matched correctly, it's correct, otherwise incorrect
    if (matchedLines.every(matched => matched)) {
      setMessage('Correct');
    } else {
      setMessage('Incorrect');
