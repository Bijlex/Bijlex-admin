...
function CreateFractionBuckets({ setCustomData, customData }) {
  const [bucketCount, setBucketCount] = useState(customData?.bucketCount || 3);
  const [fractions, setFractions] = useState(
    customData?.fractions || Array(3).fill("")
  );
  const [showBuckets, setShowBuckets] = useState(
    customData?.showBuckets || false
  );
  ...
  useEffect(() => {
    ...
    setFractions(customData?.fractions || Array(3).fill(""));
    setShowBuckets(customData?.showBuckets || false);
  ...
  const saveExercise = async () => {
    ...
    if (!allValid) {
      alert(
        "All fractions must be in the format 'numerator/denominator' (e.g., 1/2, 3/4). Please correct your inputs."
      );
      return;
    }
